  ...
  <section id="skills" class="section bg-transparent section-padding" data-section="skills">
    ...
    <div class="section-content">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-[800px] xl:max-w-[900px] 2xl:max-w-[1000px] mx-auto justify-items-center lg:justify-items-stretch">
        <div
          v-for="(category, cIndex) in skillsByCategory"
          :key="category.id"
          class="glass reveal p-3 w-full max-w-[230px] sm:max-w-none lg:w-full flex flex-col"
          :style="{ '--reveal-i': cIndex }"
        >
          <h3 class="font-(--font-display) text-sm text-[var(--color-terminal-green)] mb-1.5 pb-0.5 border-b border-[color-mix(in_srgb,var(--color-accent-secondary)_20%,transparent)]">{{ category.name }}</h3>
          ...
          <div class="flex flex-col gap-1.5 flex-1">
            <component
              ...
              class="group flex items-center gap-2 py-1.5 px-2.5 bg-black/15 rounded-md transition-[background-color,transform] duration-150 hover:bg-black/25 hover:translate-x-1"
            ...
            >
              ...
              <span v-if="skill.url" class="ml-auto text-[var(--color-text-faint)] text-xs transition-[transform,color] duration-150 group-hover:text-white/80 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">↗</span>
            </component>
          </div>
        </div>
      </div>
